...
function normalizeCreateBody(raw: unknown): { ok: true; body: CreateBody } | { ok: false; message: string } {
  ...
  const body: CreateBody = {
    ...
    description: isNonEmptyString(raw.description) ? safeString(raw.description).trim() : "",
    module: isNonEmptyString(raw.module) ? safeString(raw.module).trim() : null,
    autoTranslate: typeof raw.autoTranslate === "boolean" ? raw.autoTranslate : false,
    lastModifiedBy: isNonEmptyString(raw.lastModifiedBy) ? safeString(raw.lastModifiedBy).trim() : null,
    lastModifiedEmail: isNonEmptyString(raw.lastModifiedEmail) ? safeString(raw.lastModifiedEmail).trim().toLowerCase() : null,
  };

  return { ok: true, body };
}

function normalizeUpdateBody(raw: unknown): { ok: true; body: UpdateBody } | { ok: false; message: string } {
  if (!isObj(raw)) return { ok: false, message: "Invalid body." };
  ...
  if ("key" in raw && isNonEmptyString(raw.key)) body.key = safeString(raw.key).trim();
  if ("value" in raw) body.value = (raw as { value: unknown }).value;

  if ("description" in raw) body.description = isNonEmptyString(raw.description) ? safeString(raw.description).trim() : "";
  if ("module" in raw) body.module = isNonEmptyString(raw.module) ? safeString(raw.module).trim() : null;
  if ("autoTranslate" in raw && typeof raw.autoTranslate === "boolean") body.autoTranslate = raw.autoTranslate;

  if ("lastModifiedBy" in raw) body.lastModifiedBy = isNonEmptyString(raw.lastModifiedBy) ? safeString(raw.lastModifiedBy).trim() : null;
  if ("lastModifiedEmail" in raw)
    body.lastModifiedEmail = isNonEmptyString(raw.lastModifiedEmail) ? safeString(raw.lastModifiedEmail).trim().toLowerCase() : null;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const exists = await SystemSettings.findOne({ key: parsed.body.key }).select({ _id: 1 }).lean<{ _id: string } | null>();
    if (exists?._id) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (patch.key) {
      const collision = await SystemSettings.findOne({ key: patch.key, _id: { $ne: _id } })
        .select({ _id: 1 })
    ...
    const updated = await SystemSettings.findByIdAndUpdate(_id, patch, { new: true }).lean<LeanSetting | null>();
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const deleted = await SystemSettings.findByIdAndDelete(_id).lean<LeanSetting | null>();
    ...
    const body: ApiOk<{ success: true }> = { ok: true, data: { success: true } };
    return NextResponse.json(body, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Unknown error";
    const body: ApiFail = { ok: false, message: msg };
