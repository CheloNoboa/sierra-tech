 ...
 * - permitir administrar scheduler tipo alarma/programador
 * - permitir administrar configuración de correos
...
function normalizeIntervalUnit(
	value: unknown,
): MaintenanceSchedulerIntervalUnit {
...
function normalizeWeekdays(value: unknown): MaintenanceSchedulerWeekday[] {
	...
	return days.length > 0 ? Array.from(new Set(days)) : DEFAULT_SETTINGS.weeklyRunDays;
}
...
function normalizeSettingsPayload(value: unknown): MaintenanceSettingsPayload {
	...
	return {
		...
		intervalValue: Math.max(1, normalizeNumber(source.intervalValue, 1)),
		intervalUnit: normalizeIntervalUnit(source.intervalUnit),
		intervalStartTime: normalizeTime(
			source.intervalStartTime,
			DEFAULT_SETTINGS.intervalStartTime,
		),
		...
		timezone:
			normalizeString(source.timezone) || DEFAULT_SETTINGS.timezone,
		...
		lastRunProcessed: Math.max(
			0,
			normalizeNumber(source.lastRunProcessed, 0),
		),
...
export async function PUT(req: Request) {
	try {
		...
		settings.set({
			...
			intervalStartTime: payload.intervalStartTime,
