...
export async function PUT(req: NextRequest) {
	...
	try {
		...
		if (current.code === "superadmin" && session?.user?.role !== "superadmin") {
			return NextResponse.json(
				{
					error:
						locale === "es"
		...
		const updated = await Role.findByIdAndUpdate(id, fields, {
			new: true,
		}).exec();
...
export async function DELETE(req: NextRequest) {
	...
	try {
		...
		if (role.code === "superadmin" && session?.user?.role !== "superadmin") {
			return NextResponse.json(
				{
					error:
						locale === "es"
