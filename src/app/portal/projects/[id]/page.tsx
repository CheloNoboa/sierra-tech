...
function formatVisibleStatus(
	value: PortalProjectDetail["visibleStatus"],
): string {
	switch (value) {
		case "active":
			return "Activo";
...
function formatMaintenanceStatus(
	value: PortalMaintenanceItem["status"],
): string {
	switch (value) {
		case "scheduled":
			return "Programado";
			...
			{maintenanceItems.map((item) => (
				...
				>
					<div className="min-w-0 space-y-2">
						...
						<p className="break-words text-sm leading-7 text-text-secondary">
							{item.description ??
								"Mantenimiento visible asociado al proyecto."}
						</p>
					</div>
			...
			{alerts.map((alert) => (
				...
				>
					...
					<div className="mt-4 grid gap-3 sm:grid-cols-2">
						...
						<MetaTile label="Fecha" value={formatDateLabel(alert.dueDate)} />
					</div>
				</article>
			))}
		</div>
		...
		<div className="space-y-6">
			...
			<section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
				...
				<div className="rounded-3xl border border-border bg-white p-6 shadow-sm">
					...
					<p className="text-sm font-medium text-text-secondary">Documentos</p>
