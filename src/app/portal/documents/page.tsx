...
function matchesProjectFilter(item: PortalDocumentItem, projectId: string): boolean {
  if (!projectId || projectId === "all") return true;
  return (item.projectId ?? "") === projectId;
}

function matchesSourceFilter(item: PortalDocumentItem, source: string): boolean {
...
function countMaintenanceAttachments(items: PortalDocumentItem[]): number {
  return items.filter((item) => item.source === "maintenance_attachment").length;
}
...
function ActiveFiltersSummary({
  ...
  const selectedProject =
    projectOptions.find((option) => option.projectId === projectId)?.projectTitle ??
    "Todos los proyectos";
  ...
  const hasActiveFilters = Boolean(query || (projectId && projectId !== "all") || (source && source !== "all"));
...
export default async function PortalDocumentsPage({
  ...
  const documentsData = await getPortalDocumentsByOrganization(user.organizationId);
          ...
          <section className="rounded-[28px] border border-border bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-3">
                ...
                <div className="rounded-2xl border border-border bg-surface px-4 py-4">
                    ...
                    <p className="mt-2 text-sm leading-6 text-text-secondary">
                    Archivos documentales registrados directamente en los proyectos.
                    </p>
                </div>
            </div>
          </section>
