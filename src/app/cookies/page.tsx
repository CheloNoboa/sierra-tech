...
function getFallbackTitle(locale: Lang): string {
  return locale === "es" ? "Política de Cookies" : "Cookies Policy";
}
    ...
    async function fetchData() {
      try {
        ...
        const [policyRes, updateRes] = await Promise.all([
          fetch(ROUTES.API.COOKIES_PUBLIC(lang)),
          fetch(ROUTES.API.COOKIES_LAST_UPDATE(lang)),
        ]);
  ...
  return (
    ...
    >
      {hasServerError ? (
        <div className="mx-auto flex min-h-[36vh] max-w-2xl items-center justify-center">
          <div className="w-full rounded-2xl border border-border bg-surface px-6 py-8 text-center shadow-sm">
            ...
            <p className="text-text-secondary">
              {lang === "es"
                ? "Ocurrió un problema al consultar la política de cookies. Inténtalo nuevamente en unos minutos."
                : "There was a problem loading the cookies policy. Please try again in a few minutes."}
            </p>
          </div>
        </div>
      ) : isEmptyState ? (
        <div className="mx-auto flex min-h-[36vh] max-w-2xl items-center justify-center">
          <div className="w-full rounded-2xl border border-border bg-surface px-6 py-8 text-center shadow-sm">
            <Cookie className="mx-auto mb-4 h-10 w-10 text-text-secondary" />
            <h2 className="mb-2 text-xl font-semibold text-text-primary">
              {lang === "es" ? "Contenido en actualización" : "Content is being updated"}
            </h2>
            <p className="mx-auto max-w-xl leading-relaxed text-text-secondary">
              {lang === "es"
                ? "Estamos actualizando la Política de Cookies. Muy pronto esta información estará disponible."
                : "We are updating the Cookies Policy. This information will be available soon."}
            </p>
          </div>
        </div>
      ) : hasSections ? (
        ...
        <div className="mx-auto flex min-h-[28vh] max-w-2xl items-center justify-center">
          <div className="w-full rounded-2xl border border-border bg-surface px-6 py-8 text-center shadow-sm">
            ...
            <p className="mx-auto max-w-xl leading-relaxed text-text-secondary">
              {lang === "es"
                ? "La Política de Cookies existe, pero todavía no tiene contenido visible para este idioma."
                : "The Cookies Policy exists, but it does not have visible content for this language yet."}
            </p>
          </div>
        </div>
      ) : null}
