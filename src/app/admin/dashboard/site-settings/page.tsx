...
import type { Locale, SiteSettingsPayload } from "@/lib/site-settings.contract";
...
export default function SiteSettingsPage() {
	...
	const [siteForm, setSiteForm] = useState<SiteSettingsPayload>(
		SITE_SETTINGS_DEFAULTS,
	);
	const [siteInitialData, setSiteInitialData] = useState<SiteSettingsPayload>(
		SITE_SETTINGS_DEFAULTS,
	);
	...
	async function handleLogoDarkUpload(
		...
		try {
			...
			toast.error(lang === "es" ? "Error al subir el logo." : "Upload error.");
		} finally {
	...
	async function handleFaviconUpload(
		...
		try {
			...
			toast.error(
				lang === "es" ? "Error al subir el favicon." : "Upload error.",
			);
		} finally {
	...
	async function handleOgImageUpload(
		...
		try {
			...
			toast.error(
				lang === "es" ? "Error al subir la imagen OG." : "Upload error.",
			);
		} finally {
				...
				<div className="grid gap-5 md:grid-cols-2">
					<div>
						...
						<TextInput
							...
							onChange={(e) =>
								updateSocialLinkField("facebook", e.target.value)
							}
						/>
					</div>
					...
					<div>
						...
						<TextInput
							...
							onChange={(e) =>
								updateSocialLinkField("linkedin", e.target.value)
							}
						/>
					</div>
				...
				<div className="grid gap-5 md:grid-cols-2">
					<div>
						...
						<select
							...
							onChange={(e) =>
								updateI18nDefaultLocale(e.target.value as Locale)
							}
							className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm text-text-primary outline-none transition focus:border-brand-primaryStrong"
						>
