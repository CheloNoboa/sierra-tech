...
export default function ActivateAccountClient() {
  ...
  const passwordRuleError = useMemo(() => validatePassword(password), [password]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!token) {
      setStatus("error");
      setErrorMessage("El enlace de activación no es válido o está incompleto.");
      return;
    }
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div>
                ...
                {password.length > 0 && passwordRuleError ? (
                  <p className="mt-2 text-xs text-red-600">{passwordRuleError}</p>
                ) : null}
              </div>
              ...
              {status === "success" ? (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">
                  Su cuenta ha sido activada correctamente. Redirigiendo al inicio
                  de sesión...
                </div>
              ) : null}
