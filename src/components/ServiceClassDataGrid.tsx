...
export default function ServiceClassDataGrid() {
	...
	const t = useMemo(
		() => ({
			...
			saveError:
				lang === "es" ? "Error guardando información." : "Error saving data.",
			deleteError:
				lang === "es" ? "Error eliminando registro." : "Error deleting record.",
			duplicateError:
				lang === "es" ? "La key ya existe." : "The key already exists.",
	...
	const [deleteTarget, setDeleteTarget] = useState<ServiceClassDTO | null>(
		null,
	);
	...
	const loadGrid = useCallback(
		async (showGridLoading: boolean): Promise<void> => {
			...
			try {
				...
				const msg = e instanceof Error ? e.message : textRef.current.loadError;
