...
function isLocalizedText(value: unknown): value is LocalizedText {
	return (
		isObj(value) && typeof value.es === "string" && typeof value.en === "string"
	);
}
...
function isProjectEntity(value: unknown): value is ProjectEntity {
	...
	return (
		...
		(typeof value.primaryClientId === "string" ||
			value.primaryClientId === null) &&
...
function getTechnologySearchText(value: unknown, locale: "es" | "en"): string {
	...
	if (isObj(value) && Array.isArray(value.es) && Array.isArray(value.en)) {
...
function formatProjectDate(value: string | null, locale: "es" | "en"): string {
	if (!value) return "—";
...
export default function ProjectsDataGrid() {
	...
	const t = useMemo(
		() => ({
			title:
				safeLocale === "es" ? "Gestión de Proyectos" : "Projects Management",
			subtitle:
				safeLocale === "es"
			...
			allVisibility:
				safeLocale === "es" ? "Todas las visibilidades" : "All visibility",
			allStatuses: safeLocale === "es" ? "Todos los estados" : "All statuses",
			allFeatured: safeLocale === "es" ? "Todos" : "All",

			featuredOnly: safeLocale === "es" ? "Solo destacados" : "Featured only",
			notFeaturedOnly: safeLocale === "es" ? "No destacados" : "Not featured",
			...
			loading:
				safeLocale === "es" ? "Cargando proyectos..." : "Loading projects...",
	...
	const filtered = useMemo(() => {
		return projects.filter((project) => {
			...
			const systemType = getMixedText(
				project.systemType,
				safeLocale,
			).toLowerCase();
			const treatedMedium = getMixedText(
				project.treatedMedium,
				safeLocale,
			).toLowerCase();
