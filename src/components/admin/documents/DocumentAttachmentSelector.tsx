...
function getDocumentTitle(title: LocalizedText, locale: Locale = "es"): string {
	if (locale === "en") {
...
function toQuickFormState(
	document: AdminDocumentListItem,
): QuickDocumentFormState {
	return {
		title: {
...
export default function DocumentAttachmentSelector({
	...
	const copy = useMemo(() => {
		return {
			...
			selected: locale === "es" ? "Documentos asociados" : "Attached documents",
			available:
				locale === "es" ? "Documentos disponibles" : "Available documents",
			duplicate:
				locale === "es"
			...
			openDocument: locale === "es" ? "Abrir documento" : "Open document",
			editDocument: locale === "es" ? "Editar documento" : "Edit document",
			untitled: locale === "es" ? "Sin título" : "Untitled",
			refresh: locale === "es" ? "Recargar documentos" : "Refresh documents",
			...
			quickEditSave: locale === "es" ? "Guardar cambios" : "Save changes",
			quickSaving: locale === "es" ? "Guardando..." : "Saving...",
	...
	async function handleQuickDocumentFileUpload(
		...
		try {
			...
			console.error(
				"[DocumentAttachmentSelector] document upload error:",
				error,
			);
	...
	async function handleQuickThumbnailUpload(
		...
		try {
			...
			console.error(
				"[DocumentAttachmentSelector] thumbnail upload error:",
				error,
			);
	...
	const handleQuickSave = async (): Promise<void> => {
		...
		try {
			...
			const response = await fetch(url, {
				...
				body: JSON.stringify(
					buildQuickDocumentPayload(quickForm, relatedModule),
				),
			});

			const json: DocumentMutationApiResponse = await response
				.json()
				.catch(() => ({
					ok: false,
							...
							<div className="space-y-3">
								{availableDocuments.map((document) => {
									...
									return (
										<article
											key={
												document._id || `${document.fileUrl}-${document.type}`
											}
					...
					<div>
						...
						<div className="space-y-3">
							<input
								...
								onChange={(e) =>
									handleQuickChange("thumbnailUrl", e.target.value)
								}
						...
						<div>
							...
							<select
								...
								onChange={(e) =>
									handleQuickChange(
										"language",
										e.target.value as DocumentLanguage,
									)
								}
								className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-lime-500"
							>
						...
						<div className="flex items-end">
							<label className="inline-flex items-center gap-3 rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-800">
								<input
									...
									onChange={(e) =>
										handleQuickChange("featured", e.target.checked)
									}
