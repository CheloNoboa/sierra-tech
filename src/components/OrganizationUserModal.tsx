...
function normalizeSavedUser(
	...
	if (context.existingUser) {
		...
		const merged: OrganizationUserRow = {
			...
			firstName:
				getString(source?.firstName) || context.values.firstName.trim(),
			...
			createdAt: getString(source?.createdAt) || context.existingUser.createdAt,
			updatedAt: getString(source?.updatedAt) || context.existingUser.updatedAt,
			organizationName:
