...
function hasContractStartedBeforeToday(
...
export default function ProjectModal({
	...
	const t = useMemo(
		() => ({
			...
			maintenance: safeLocale === "es" ? "Mantenimientos" : "Maintenance",
			maintenanceNote:
				safeLocale === "es"
			...
			contract: safeLocale === "es" ? "Contrato base" : "Base contract",
			contractNote:
				safeLocale === "es"
			...
			try {
				...
				const mapped = source
					.map((item) => ({
						...
						email:
							normalizeString(
								(item as RawOrganization & { primaryEmail?: string })
									.primaryEmail,
							) || normalizeString(item.email),
					}))
	...
	async function handleSave() {
		...
		const resolvedClientEmail =
			normalizeString(basePrepared.clientEmail) ||
			normalizeString(selectedOrganization?.email);
		...
		const prepared: ProjectPayload = {
			...basePrepared,
			clientEmail: resolvedClientEmail, // 🔥 FIX REAL
			maintenanceItems: rebuildMaintenanceItems(
				{
					...basePrepared,
					clientEmail: resolvedClientEmail, // 🔥 IMPORTANTE también aquí
				},
				selectedOrganization?.email,
				{
					forceRegenerateSchedule: false,
	...
	function patchMaintenanceScheduleDate(
		...
		setForm((current) => {
			...
			return {
				...nextForm,
				maintenanceItems: nextForm.maintenanceItems.map(
					(maintenanceItem, idx) =>
						idx === maintenanceIndex
							? nextMaintenanceItems[maintenanceIndex]
							: maintenanceItem,
				),
			};
		});
			...
			>
				{loading ? (
					...
					<div className="space-y-6">
											...
											<button
												...
												onClick={() =>
													patch(
														"slug",
														slugify(
															form.title.es || form.title.en || form.slug,
														),
													)
												}
										...
										<select
											...
											onChange={(e) => {
												const nextId = e.currentTarget.value
													...
													: null;
												const selected =
													organizations.find((item) => item.id === nextId) ??
													null;
								...
								<div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
									<div>
										...
										<input
											...
											onChange={(e) => {
												const nextStartDate = toNullableIsoDate(
													e.currentTarget.value,
												);
									...
									<div>
										...
										<input
											...
											onChange={(e) => {
												const nextDuration = toNullableNumber(
													e.currentTarget.value,
												);
						...
						<SectionCard
							title={t.portalCategory}
							subtitle={t.portalCategoryNote}
						>
							<div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
								<div>
									...
									<input
										...
										placeholder="Ej. Potabilización de agua con ozono"
										className="w-full rounded-xl border border-border bg-surface px-3 py-2.5 text-sm text-text-primary outline-none focus:border-brand-primaryStrong focus:ring-2 focus:ring-brand-secondary"
									/>
								</div>
								...
								<div>
									...
									<input
										...
										placeholder="Ex. Ozone-based water treatment"
										className="w-full rounded-xl border border-border bg-surface px-3 py-2.5 text-sm text-text-primary outline-none focus:border-brand-primaryStrong focus:ring-2 focus:ring-brand-secondary"
									/>
								</div>
									...
									<input
										...
										onChange={(e) =>
											patch("technologyUsed", {
												es: parseCommaSeparatedValues(e.currentTarget.value),
										...
										placeholder={
											safeLocale === "es"
												? "Ej. Ozono, Filtración, Desinfección"
												: "Ex. Ozone, Filtration, Disinfection"
										}
										className="w-full rounded-xl border border-border bg-surface px-3 py-2.5 text-sm text-text-primary outline-none focus:border-brand-primaryStrong focus:ring-2 focus:ring-brand-secondary"
									/>
								</div>
										...
										onChange={(e) =>
											patch("technologyUsed", {
												es: Array.isArray(form.technologyUsed?.es)
													...
													: [],
												en: parseCommaSeparatedValues(e.currentTarget.value),
											})
										}
										placeholder={
											safeLocale === "es"
												? "Ej. Ozone, Filtration, Disinfection"
												: "Ex. Ozone, Filtration, Disinfection"
										}
										className="w-full rounded-xl border border-border bg-surface px-3 py-2.5 text-sm text-text-primary outline-none focus:border-brand-primaryStrong focus:ring-2 focus:ring-brand-secondary"
									/>
						...
						<SectionCard title={t.media} subtitle={t.mediaNote}>
							<div className="grid gap-6 xl:grid-cols-2">
								<ProjectImageUploader
									label={
										safeLocale === "es" ? "Portada principal" : "Main cover"
									}
									...
									onUpload={(file) =>
										uploadProjectAsset(file, "projects/covers")
									}
								/>
								...
								<ProjectGalleryUploader
									...
									onUpload={(file) =>
										uploadProjectAsset(file, "projects/gallery")
									}
								/>
							</div>
						</SectionCard>
																...
																<label className="inline-flex cursor-pointer items-center justify-center rounded-xl border border-border bg-surface px-4 py-2.5 text-sm font-medium text-text-primary transition hover:bg-surface-soft">
																	<input
																		...
																		onChange={async (e) => {
																			...
																			try {
																				await handleDocumentFileSelected(
																					index,
																					file,
																				);
																			} finally {
																		...
																		{previewSchedule.map(
																			(event, scheduleIndex) => (
																				...
																				>
																					...
																					<td className="px-3 py-3 text-text-primary">
																						{resolveChannelsLabel(
																							event.channels,
																							safeLocale,
																						)}
																					</td>
																					<td className="px-3 py-3 text-text-primary">
																						{resolveRecipientsLabel(
																							event.recipients,
																							safeLocale,
																						)}
																					</td>
														...
														<div className="grid grid-cols-1 gap-2 xl:grid-cols-2">
															{form.documents.length === 0 ? (
																...
																form.documents.map((doc) => {
																	const checked =
																		item.relatedDocumentIds.includes(
																			doc.documentId,
																		);
														...
														<div className="rounded-2xl border border-dashed border-border bg-surface p-4">
															<input
																...
																onChange={async (e) => {
																	...
																	try {
																		const uploaded =
																			await uploadMaintenanceFiles(files);
